	...
	public void onUpdateReceived(Update update) {
		...
		if(update.hasInlineQuery()) {
			...
			if(!isRegistered) {
				...
				InlineQueryResult iqr= new InlineQueryResultArticle().setId(inlineQueryId+101).setInputMessageContent(imc).setTitle("You are not registered");
				
				reply.setInlineQueryId(inlineQueryId)
				.setResults(iqr)
				.setPersonal(true)
				.setCacheTime(0)
